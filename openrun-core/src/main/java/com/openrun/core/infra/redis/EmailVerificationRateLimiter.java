...
public class EmailVerificationRateLimiter {
    ...
    // 클라우드 환경에서는 X-Forwarded-For로 진짜 IP를 가져와야 하고, Cloudflare나 AWS ALB 사용 시 해당 헤더 처리
    // 로직을 조금 더 보강해야함
    // 왜냐? 클라우드 환경에서는 로드밸런서나 CDN과같은 프록시서버가 존재하기 때문에 request.getRemoteAddr(); 으로 IP를
    // 가져오면 클라이언트의 IP가 아닌 프록시서버의 IP를 가져오게 됨.
    // 클라우드 환경 도입 시 변경 예정
